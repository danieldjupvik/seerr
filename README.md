...
This is a temporary bridge until PR #1855 merges upstream — see [issue #1048](https://github.com/seerr-team/seerr/issues/1048).

## Image

```
ghcr.io/danieldjupvik/seerr-hide-requested:latest
```
...
1. Checks whether PR #1855 is still open upstream. If it was **merged or closed**, the workflow opens an issue in this repo telling you to switch back to the official image, disables itself, and fails — that issue is your signal to retire this repo.
...
GitHub profile → Packages → `seerr-hide-requested` → Package settings → Change visibility → Public.
...
| PR diff conflicts with a new release | Issue opened, build fails, previous image stays available. You're behind upstream until the PR is rebased or you retire this repo. |
